...
//                System.out.println(filePath);
    ...
    private void loadDi() {
        ...
        for (Map.Entry<Class, Object> entry : entries) {
            ...
            //4 判断属性上面是否有@Di注解
                Di annotation = file.getAnnotation(Di.class);
                if (annotation!=null){
                    //如果私有属性，设置可以设置值
                    file.setAccessible(true);
                    //5 如果有注解@Di，吧对象进行设置（注入）
                    try {
                        file.set(obj,beanFactory.get(file.getType()));
    ...
    private void loadBean(File file) throws Exception {
        ...
        if (file.isDirectory()) {
            ...
            for (File child : childrenFiles) {
                //4.1遍历得到每个file对象，继续判断，如果还是文件，继续递归
